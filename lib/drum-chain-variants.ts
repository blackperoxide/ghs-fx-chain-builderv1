...
import type { Axis, AxisScores } from "./chain-axes"
import { axisDistance } from "./chain-axes"

export interface StageSlotVariant {
  /** Stable id, reused as the ChainStage id (feeds stageCategoryMap). */
  id: string
  name: string
  role: string
  options: PluginOption[]
  note?: string
  /** Where this variant sits in axis-space, only on the axes this slot cares about. */
  center: Partial<AxisScores>
}

interface StageSlot {
  /** Display name prefix, e.g. "Transient Shaping" - numbered automatically at build time. */
  label: string
  /** Which axes matter for picking a variant in this slot. */
  axes: Axis[]
  /** Whether this slot appears at all for a given axis point - every chain needs the core 4; the rest are conditional, matching how live-organic's chain already skips saturation/modulation/glitch/delay entirely. */
  include: (axes: AxisScores) => boolean
  variants: StageSlotVariant[]
}

// Every option/tip below is reused verbatim from the original 5 hand-written
// drumBusChain vibes (lib/chain-data.ts) - this is a reorganization of that
// content by axis-character, not new material. Axis centers are best-effort
// judgment calls grounded in each variant's own description, not measurements.
export const drumStageSlots: StageSlot[] = [
  {
    ...
    variants: [
      ...
      {
        id: "transient",
        name: "Exaggerate Before Smashing It",
        role: "Exaggerate the hit before you smash it",
        center: { punch: 3, drive: 9 },
        options: [
      ...
      {
        id: "transient",
        name: "Light Transient Touch",
        role: "Keep it natural — you're playing live",
        center: { punch: 6, drive: 1 },
        options: [
  ...
  },
  {
    label: "Saturation",
    axes: ["drive", "lofi"],
    include: (a) => a.drive >= 3 || a.lofi >= 3,
    variants: [
      {
        id: "saturation",
        name: "Saturation / Grit",
        role: "This is the character stage — where 'Deftones drum bus' lives",
        center: { drive: 9, lofi: 6 },
        options: [
  ...
  },
  {
    ...
    variants: [
      ...
      },
      {
        id: "eq",
        name: "Warm Body EQ",
        role: "Boost the low-mid body that makes the drums feel 'thick,' not muddy",
        center: { brightness: 4 },
        options: [
      ...
      {
        id: "eq",
        name: "EQ for Live Cut-Through",
        role: "Carve room for the kit in a live mix",
        center: { brightness: 5 },
        options: [
  ...
  {
    label: "Modulation / Movement",
    axes: ["movement", "space"],
    include: (a) => a.movement >= 4,
    variants: [
      {
        id: "modulation",
        name: "Modulation / Movement",
        role: "The psychedelic swirl",
        center: { movement: 8, space: 8 },
        options: [
  ...
  },
  {
    label: "Glitch Send",
    axes: ["movement", "lofi"],
    include: (a) => a.movement >= 3 || a.lofi >= 3,
    variants: [
      {
        id: "glitch-fx",
        name: "Glitch Send (subtle)",
        role: "One or two stutter/reverse moments per bar, on a send",
        center: { movement: 2, lofi: 2 },
        options: [
      ...
      {
        id: "glitch-fx",
        name: "Glitch / Stutter Send",
        role: "Chaotic breaks, reversed hits, filter chokes",
        center: { movement: 8, lofi: 8 },
        options: [
  ...
  },
  {
    ...
    variants: [
      ...
      },
      {
        id: "delay-fx",
        name: "Delay",
        role: "Trip-hop haze layered on top of the foundation",
        center: { space: 4, movement: 3 },
        options: [
          { brand: "FabFilter", plugin: "Timeless 2", tip: "Dotted-8th, filtered repeats, moderate feedback — same washy-transition move, used more sparingly." },
        ],
      },
    ],
  },
  {
    ...
    variants: [
      ...
      {
        id: "space",
        name: "Space (Wide, Hazy)",
        role: "Wide, hazy, but controlled",
        center: { space: 9, width: 8 },
        options: [
      ...
      },
      {
        ...
        center: { space: 5, width: 5 },
        options: [
          { brand: "UAD", plugin: "Capitol Chambers", tip: "Shorter decay than a wide wash (0.8-1.4s), moderate pre-delay, low-passed return." },
        ],
      },
      {
        id: "space",
...
function pickVariant(slot: StageSlot, axes: AxisScores): StageSlotVariant {
  let best = slot.variants[0]
  let bestDist = Infinity
  for (const variant of slot.variants) {
    const dist = axisDistance(axes, variant.center, slot.axes)
    if (dist < bestDist) {
      bestDist = dist
      best = variant
    }
...
/** Assembles a drum chain from the current axis point - the generalized replacement for drumBusChain[vibe]. */
export function buildDrumChain(axes: AxisScores): BuiltStage[] {
  const included = drumStageSlots.filter((slot) => slot.include(axes))

  return included.map((slot, i) => {
    const variant = pickVariant(slot, axes)
    return {
      id: variant.id,
      name: `${i + 1}. ${variant.name}`,
      role: variant.role,
